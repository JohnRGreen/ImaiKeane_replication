...
def solveProblem_step6(M_grid, k_grid, **params_sp):
    ...
    all_nan_columns = np.all(np.isnan(nonsoncstrained), axis=0)
    mincolval = np.full(all_nan_columns.shape, np.nan)
    mincolval[~all_nan_columns] = np.nanmin(nonsoncstrained[:,~all_nan_columns], axis=0)
    ...
    kforinterp = ~np.isnan(mincolval) | colarg < (nK-5)
    # get first row for extrapolation
    # if there are some bad rows, we split things up
    if sum(kforinterp)<nK:
        kstart = np.argmax(~kforinterp)
    else:
        kstart = nK
    ...
    for ixk in range(kstart,nK):
        # print(ixk)
        # increase h by a small number
        policyH[1:(nM+1), ixk+1, ixt] = 1.01*policyH[1:(nM+1), ixk, ixt]
        # increase c by small number
        cons = np.minimum(1,1.01* policyC[1:(nM+1), ixk, ixt]/policyM[1:(nM+1),ixt])
        policyC[1:(nM+1), ixk+1, ixt] = cons*policyM[1:(nM+1),ixt]
    ...
    for ixt in range(lifespan-3, -1, -1):
        ...
        ECprimesub[egmmask[:,:,ixt+1] == 0] = np.nan
        ...
        for ixk in range(kstart,nK):
            # print(ixk)
            # increase h by a small number
            policyH[:(nM+1), ixk+1, ixt] = 1.01*policyH[:(nM+1), ixk, ixt]
            # increase c by small number
            cons = np.minimum(1,1.01* policyC[1:(nM+1), ixk, ixt]/policyM[1:(nM+1),ixt])
            policyC[1:(nM+1), ixk+1, ixt] = cons*policyM[1:(nM+1),ixt]
        # fill in M = 0 C
